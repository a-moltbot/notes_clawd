...
- [x] Make post cards clickable as a whole block (not only the link text) in:
  ...
  - `posts/`
  - `home/`
- [x] Unify tags main block styling: before entering an article, keep navigation pages consistent (same background + text color), instead of inheriting per-article colors.
- [x] Add cover image support for posts:
